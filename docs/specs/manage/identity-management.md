...
## Overview
Identity Management is the central person directory for Duall Master 3.0 — the single source of truth for every individual who interacts with a building. It manages person profiles, credentials (face templates, cards, PINs, QR codes, mobile BLE), department/group structures, and orchestrates credential synchronization to access control devices. All access decisions happen locally on devices; this service ensures devices have up-to-date person and credential data.
...
| person_type | PersonTypeEnum | yes | employee | Type of person |
...
| head_person_id | uuid | no | — | FK to Person (department head) |
...
| id | uuid | yes | auto | Primary key |
| person_id | uuid | yes | — | FK to Person |
| group_id | uuid | yes | — | FK to PersonGroup |
...
| person_id | uuid | yes | — | FK to Person (denormalized for speed) |
...
- **Query params:**
  ...
  | group_id | uuid | — | Filter by person group |
...
- **Body:**
  ...
  ```
- **Side effects:** audit log `person.created`, NATS publish `dm.{tenant}.identity.person.created`
- **Response 201:** Created Person resource

### GET /api/v1/identity/persons/{id}
- **Auth:** role >= viewer
- **Response 200:** Full person detail with credentials summary, groups, recent events

### PUT /api/v1/identity/persons/{id}
- **Auth:** role >= operator
- **Body:** Partial update fields
- **Side effects:** audit log `person.updated`, if status changed → NATS `dm.{tenant}.identity.person.status_changed`, trigger credential sync if relevant fields changed

### DELETE /api/v1/identity/persons/{id}
- **Auth:** role >= admin
- **Side effects:** Soft-delete; revokes all credentials; publishes `person.deleted`; triggers credential removal sync to all devices
- **Response 200:** `{ "deleted": true }`

### POST /api/v1/identity/persons/{id}/credentials
- **Auth:** role >= operator
- **Body:**
  ```json
  {
  ...
  ```
- **Side effects:** audit log `credential.created`, triggers sync to all devices person has access to via MQTT
- **Response 201:** Created credential

### GET /api/v1/identity/persons/{id}/credentials
- **Auth:** role >= viewer
- **Response 200:** List of credentials for person

### PUT /api/v1/identity/persons/{person_id}/credentials/{cred_id}
- **Auth:** role >= operator
- **Side effects:** Increments `sync_version`, triggers re-sync to devices

### DELETE /api/v1/identity/persons/{person_id}/credentials/{cred_id}
- **Auth:** role >= operator
- **Side effects:** Sets status to `revoked`, triggers credential removal from all devices

### POST /api/v1/identity/persons/{id}/credentials/face/enroll
- **Auth:** role >= operator
...
### POST /api/v1/identity/persons/{id}/suspend
- **Auth:** role >= admin
- **Body:** `{ "reason": "Điều tra nội bộ" }`
- **Side effects:** Sets person status to `suspended`, suspends all credentials, triggers immediate credential removal from all devices (priority sync), audit log

### POST /api/v1/identity/persons/{id}/reactivate
- **Auth:** role >= admin
- **Side effects:** Sets person status to `active`, reactivates previously-active credentials, triggers sync

### POST /api/v1/identity/persons/import
- **Auth:** role >= admin
- **Body:** `multipart/form-data` with CSV/Excel file
- **Processing:** Validates, creates persons in batch, returns summary with errors
...
- **Auth:** role >= operator
- **Body:** `{ "person_ids": ["uuid1", "uuid2"], "valid_until": "2026-12-31T23:59:59Z" }`
- **Side effects:** Updates group membership, triggers access rule re-evaluation and credential sync for affected persons

### DELETE /api/v1/identity/groups/{id}/members
- **Auth:** role >= operator
- **Body:** `{ "person_ids": ["uuid1"] }`

### GET /api/v1/identity/sync/status
- **Auth:** role >= admin
- **Query params:** `site_id`, `device_id`, `person_id`
...
|-------|-----------|-----|----------------|-------------|
| `dm/{tenant}/device/{device_id}/cfg/persons` | server→device | 1 | `{ "action": "add\|update\|delete", "person_id": "uuid", "credentials": [...], "sync_version": 42 }` | Push person credentials to device |
| `dm/{tenant}/device/{device_id}/cfg/persons/bulk` | server→device | 1 | `{ "persons": [...], "sync_version": 42 }` | Bulk sync (initial or full re-sync) |
| `dm/{tenant}/device/{device_id}/cfg/persons/ack` | device→server | 1 | `{ "person_id": "uuid", "sync_version": 42, "status": "ok\|error", "error": "..." }` | Device confirms credential sync |
| `dm/{tenant}/device/{device_id}/evt/access` | device→server | 1 | `{ "person_id": "uuid", "credential_type": "face", "decision": "granted", "time": "..." }` | Access event from device |

## Business Rules

1. **BR-ID-001: Unique credential per type per person.** A person can have at most one active credential of each type (one active card, one active face template, one active PIN). Creating a new one of the same type auto-revokes the previous one.
...
6. **BR-ID-006: Suspend cascades to credentials.** Suspending a person immediately suspends all their credentials and triggers priority sync (within 5 seconds for online devices).
...
8. **BR-ID-008: Department deletion blocked if members exist.** Cannot delete a department that has active persons. Must reassign persons first.

9. **BR-ID-009: Group membership with access rules.** When a person is added to a group that has associated access rules, credential sync is triggered for all doors in those rules.
...
- **Device-side:** Each access control device stores a local person database with credential data (face templates, card UIDs, PIN hashes). Devices make all access decisions locally — the server is NEVER in the decision loop. When a device is offline, it continues granting/denying access based on its local database.
- **Sync strategy:** Server pushes credential changes to devices via MQTT. Each credential change increments a monotonic `sync_version`. When a device reconnects after being offline, it reports its last `sync_version` per person. The server computes the delta and pushes only changed records. For initial provisioning or corrupted state, a full bulk sync can be triggered.
- **Conflict resolution:** Server-wins. The server's person database is authoritative. If a device has stale data, the server's version always overwrites. Sync versions ensure ordering.
- **Local storage:** Devices typically store 10,000–50,000 person records (face templates ~2KB each, card UIDs ~100B each). Face templates are compressed. A site with 5,000 persons uses ~12MB of device storage for face templates.
...
|-------|------|----------------|
| /manage/identities | Person list | DataTable with search, department/status filters, credential icons, bulk actions |
| /manage/identities/:id | Person detail | Profile header, credential tabs (Card/Face/PIN/Mobile), group membership, access history timeline, sync status |
| /manage/identities/new | Create person | Multi-step form: basic info → department → credentials → groups |
...
|------------|---------|---------|-----------|
| person.created | POST create | full resource | 7 years |
| person.updated | PUT update | diff only | 7 years |
| person.suspended | POST suspend | id + reason + actor | 7 years |
| person.terminated | POST terminate | id + reason + actor | permanent |
| person.reactivated | POST reactivate | id + actor | 7 years |
| person.deleted | DELETE | id + actor | permanent |
| credential.created | POST credential | type + person_id + actor | 7 years |
| credential.revoked | DELETE credential | type + person_id + actor | 7 years |
| credential.expired | auto job | type + person_id | 7 years |
| credential.synced | device ack | device_id + person_id + version | 1 year |
| credential.sync_failed | device nack | device_id + person_id + error | 1 year |
| group.member_added | POST members | group_id + person_ids | 7 years |
| group.member_removed | DELETE members | group_id + person_ids | 7 years |
| person.imported | POST import | count + filename + actor | 7 years |
...
- **Depends on:**
  ...
  - `access-svc` — access rules that reference person groups (gRPC query)
  - `tenant-svc` — tenant configuration, site hierarchy
- **Consumed by:**
  - `access-svc` — queries person/credential data for rule management and analytics
  - `visitor-svc` — creates temporary person records with visitor type
  - `attend-svc` — reads person data for attendance records
  - `report-svc` — person analytics, department reports
...
- The system supports multi-site persons (e.g., an employee with access to Tòa nhà Landmark 72 and Tòa nhà Vietcombank Tower). Each site maintains its own credential sync state.
...
- For GDPR/PDPA compliance, person data export and deletion APIs exist but are controlled by super_admin role.
