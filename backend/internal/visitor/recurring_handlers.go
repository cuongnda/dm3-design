...
func (h *VisitorHandlers) ListRecurringTemplates(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Pool.Query(r.Context(), `
		SELECT t.id, t.tenant_id, t.visitor_id, COALESCE(t.host_user_id::text,''), t.purpose,
...
func (h *VisitorHandlers) CreateRecurringTemplate(w http.ResponseWriter, r *http.Request) {
	...
	if req.VisitorID == "" || req.Purpose == "" || req.RecurrenceRule == "" || req.StartDate == "" {
		httputil.Error(w, http.StatusBadRequest, "visitor_id, purpose, recurrence_rule, and start_date are required")
		return
	}

	// Host requirement mirrors the single-visit gate: only required when the
	// tenant's approval workflow is enabled.
	settings, err := h.getOrCreateSettings(r.Context(), cid)
	if err != nil {
		slog.Error("create recurring: load visitor settings error", "error", err, "tenant_id", cid)
	...
	err = h.db.Pool.QueryRow(r.Context(), `
		...
		VALUES ($1::uuid, $2::uuid, NULLIF($3,'')::uuid, $4, $5::uuid[], $6, $7, $8::date, $9::date, $10::uuid)
		RETURNING id, tenant_id, visitor_id, COALESCE(host_user_id::text,''), purpose,
