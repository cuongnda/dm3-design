...
export function useZones(): UseZonesReturn {
  ...
  const fetchZones = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiFetch<ZonesResponse>(`/api/v1/zones?${params}`);
  ...
  const createZone = useCallback(async (data: ZoneFormData): Promise<boolean> => {
    try {
      ...
      await apiFetch<Zone>('/api/v1/zones', {
        method: 'POST',
  ...
  const updateZone = useCallback(async (id: string, data: ZoneFormData): Promise<boolean> => {
    try {
      const payload: ZoneFormData = {
        ...data,
        parent_id: data.parent_id || undefined,
      ...
      await apiFetch<Zone>(`/api/v1/zones/${id}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      });
      await fetchZones();
  ...
  const deleteZone = useCallback(async (id: string): Promise<boolean> => {
    try {
      await apiFetch<void>(`/api/v1/zones/${id}`, { method: 'DELETE' });
      await fetchZones();
      return true;
    } catch (err) {
