...
import {
  Shield, Plus, Search, MoreHorizontal,
  Eye, Edit, Trash2,
} from 'lucide-react';
import {
  Button, Input, Label,
...
export function AccessGroupsPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      await apiFetch(`/api/v1/access-groups/${groupToDelete.id}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = async () => {
    try {
      await apiFetch('/api/v1/access-groups/bulk-delete', { method: 'POST', body: JSON.stringify({ ids: selected }) });
      setSelected([]);
      fetchAccessGroups();
    } catch (err) {
  ...
  const columns = useMemo((): Column<AccessGroup>[] => [
    {
      key: 'name',
        ...
        <div onClick={(e) => e.stopPropagation()}>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => { setGroupToDelete(g); setShowDeleteDialog(true); }}
  ...
  const GroupFormContent = (
    <div className="flex flex-col gap-4">
      <div>
        <Label htmlFor="group-name">{t('form.name', 'Name')} <span className="text-destructive">*</span></Label>
        <Input
          id="group-name"
          value={formData.name}
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 basis-0 flex-col gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-[18px] font-semibold text-foreground">
            {t('title', 'Access Groups')}
          </h1>
          <p className="text-[13px] text-muted-foreground">
            {t('description', 'Manage access groups and their assigned access points')}
          </p>
        </div>
      ...
      <DataTableCard
        title={<span className="text-[14px] font-semibold">{t('title', 'Access Groups')} ({pagination.total})</span>}
      ...
      <AppModal
        ...
        onOpenChange={(open) => { if (!open) { setShowDeleteDialog(false); setGroupToDelete(null); setDeleteError(null); } }}
        title={
          <span className="flex items-center gap-2 text-destructive">
            <Trash2 size={16} />{t('deleteGroup', 'Delete Access Group')}
          </span>
        }
        size="xs"
