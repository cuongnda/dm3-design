...
export function useAccessGroups(): UseAccessGroupsReturn {
  ...
  const fetchAccessGroups = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch(`/api/v1/access-groups?${params}`);
  ...
  const createAccessGroup = useCallback(async (data: AccessGroupFormData): Promise<boolean> => {
    try {
      await apiFetch('/api/v1/access-groups', {
        method: 'POST',
  ...
  const updateAccessGroup = useCallback(async (id: string, data: AccessGroupFormData): Promise<boolean> => {
    try {
  ...
  const deleteAccessGroup = useCallback(async (id: string): Promise<boolean> => {
    try {
      await apiFetch(`/api/v1/access-groups/${id}`, { method: 'DELETE' });
      await fetchAccessGroups();
      return true;
    } catch (err) {
