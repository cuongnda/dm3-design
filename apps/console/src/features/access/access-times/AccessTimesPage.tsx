...
import {
  Clock, Plus, Search, MoreHorizontal,
  Edit, Trash2, Eye,
} from 'lucide-react';
import {
  Button, Input,
...
export function AccessTimesPage() {
  ...
  const filteredTimes = useMemo(() => {
    ...
    return accessTimes.filter(
      at => at.name.toLowerCase().includes(q) || (at.description ?? '').toLowerCase().includes(q),
    );
  }, [accessTimes, search]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      await apiFetch(`/api/v1/access-times/${timeToDelete.id}`, { method: 'DELETE' });
  ...
  const handleBulkDelete = async () => {
    try {
      await apiFetch('/api/v1/access-times/bulk-delete', { method: 'POST', body: JSON.stringify({ ids: selected }) });
      setSelected([]);
      fetchAccessTimes();
    } catch (err) {
  ...
  const columns = useMemo((): Column<AccessTime>[] => [
    {
      key: 'name',
    ...
    },
    {
      ...
      render: (at) => (
        <span className="text-[13px] text-muted-foreground">{at.timezone}</span>
      ),
    },
    {
      key: 'is_active',
    ...
    },
    {
      ...
      render: (at) => (
        <Badge variant="secondary">
          {at.slot_count ?? at.slots?.length ?? 0}
        </Badge>
      ),
    },
    {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => navigate(`/access/access-times/${at.id}`)}>
                <Eye size={14} className="mr-2" />{t('view', 'View / Edit')}
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => { setTimeToDelete(at); setShowDeleteDialog(true); }}
                className="text-destructive"
              >
                <Trash2 size={14} className="mr-2" />{t('delete', 'Delete')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 basis-0 flex-col gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-[18px] font-semibold text-foreground">
            {t('title', 'Access Times')}
          </h1>
          <p className="text-[13px] text-muted-foreground">
            {t('description', 'Manage access time schedules and time slot templates')}
          </p>
        </div>
      ...
      <DataTableCard
        title={<span className="text-[14px] font-semibold">{t('tableTitle', 'Access Times')} ({pagination.total})</span>}
      ...
      >
        {loading ? (
          ...
          <div className="py-12 text-center text-[13px] text-muted-foreground">
            {search
              ? t('noResults', 'No access times match your search')
              : t('empty', 'No access times yet. Create the first one.')}
          </div>
        ) : (
          <DataTable
            embedded
      ...
      <AppModal
        ...
        onOpenChange={(open) => { if (!open) { setShowDeleteDialog(false); setTimeToDelete(null); setDeleteError(null); } }}
        title={
          <span className="flex items-center gap-2 text-destructive">
            <Trash2 size={16} />{t('deleteTitle', 'Delete Access Time')}
          </span>
        }
        size="xs"
