...
import { ArrowLeft, Clock, Trash2, Save, Copy, Files } from 'lucide-react';
import { Button, Input, AppModal, Select, SelectOption } from '@dm3/ui';
    ...
    return (
        ...
        >
            <div className="space-y-3">
                {local.length === 0 && <p className="text-center text-[13px] text-muted-foreground py-4">{t('form.noTimeSlots')}</p>}
                {local.map((s, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                        ...
                        <Input
                            ...
                            className="h-8 w-28 font-mono text-[12px]"
                        />
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:text-destructive" onClick={() => remove(idx)}>
                            <Trash2 size={13} />
                        </Button>
                    </div>
                ))}
        ...
        <AppModal
            ...
            title={
                <span className="flex items-center gap-2">
                    <Copy size={15} />
                    {t('form.copyTitle', { day: sourceDayLabel })}
                </span>
            }
            size="xs"
            showCancelButton
...
function DayRow({ dayOfWeek, slots, canApplyAll, onSlotsChange, onRemove, onCopy, onApplyAll }: DayRowProps) {
    ...
    const handleTimelineMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (fromMin !== toMin) {
            const updated = [...slots.slice(0, newDataIndex), { start: minToTime(fromMin), end: minToTime(toMin) }, ...slots.slice(newDataIndex)];
    ...
    return (
        <div className="mb-10">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    >
                        ...
                        {[3, 6, 9, 12, 15, 18, 21].map((h) => (
                            <div key={h} className="absolute inset-y-0 w-px bg-border/50" style={{ left: `${(h / 24) * 100}%` }} />
                        ))}
            ...
            <EditDayModal open={editOpen} onOpenChange={setEditOpen} dayOfWeek={dayOfWeek} slots={slots} onSave={onSlotsChange} />
        </div>
    );
}
...
export function AccessTimeFormPage() {
    ...
    const availableDays = useMemo(() => DAYS.filter((d) => !activeDays.includes(d.value)), [activeDays]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isNew) {
                await apiFetch('/api/v1/access/access-times', { method: 'POST', body: JSON.stringify(payload) });
            } else {
                await apiFetch(`/api/v1/access/access-times/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
            }
        ...
        >
            ...
            <div className="shrink-0">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-[18px] font-semibold text-foreground">{isNew ? t('form.titleNew') : name || '…'}</h1>
                    </div>
            ...
            <section className="shrink-0 rounded-xl border border-border bg-card px-6 py-5">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2">
                        ...
                        <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t('form.fieldRemarkPlaceholder')} />
                    </div>
            ...
            <section className="shrink-0 rounded-xl border border-border bg-card px-6 py-5 pb-6">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <h2 className="text-[15px] font-bold text-foreground">{t('form.schedule')}</h2>
                    <div className="flex items-center gap-2">
                ...
                {activeDays.length === 0 ? (
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-muted-foreground/50">{t('form.hint')}</p>
                    </div>
                )}
            </section>
