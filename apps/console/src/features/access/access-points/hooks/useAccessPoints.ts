...
export function useAccessPoints(): UseAccessPointsReturn {
  ...
  const fetchZones = useCallback(async () => {
    try {
      const data = await apiFetch<{ zones?: Zone[]; data?: Zone[] }>('/api/v1/zones?limit=100');
  ...
  const fetchAccessTimes = useCallback(async () => {
    try {
      const data = await apiFetch<{ data?: AccessTime[] }>(
        '/api/v1/access-times?limit=100',
      );
  ...
  const createAccessPoint = useCallback(
    async (data: AccessPointFormData): Promise<boolean> => {
      try {
        await apiFetch('/api/v1/access-points', {
          method: 'POST',
  ...
  const deleteAccessPoint = useCallback(
    async (id: string): Promise<boolean> => {
      try {
        await apiFetch(`/api/v1/access-points/${id}`, { method: 'DELETE' });
        await fetchAccessPoints();
        return true;
      } catch (err) {
