...
export function UserAssignModal({ isOpen, onClose, department }: UserAssignModalProps) {
  ...
  const fetchCurrentUsers = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/departments/${department.id}/users`, {
  ...
  const handleRemoveUser = async (userId: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/departments/${department.id}/users/${userId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
      if (!response.ok) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl flex flex-col" style={{ maxHeight: '85vh' }}>
        ...
        <Tabs
          value={activeTab}
          onValueChange={(v) => { setActiveTab(v as 'current' | 'assign'); setSearchTerm(''); }}
          className="flex flex-col flex-1 min-h-0"
        >
          {/* Toolbar: search + tabs + action */}
                    ...
                    <TableRow key={user.id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-[13px] font-medium">{user.first_name} {user.last_name}</div>
