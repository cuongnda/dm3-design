...
import { AppModal, Input, Label, Textarea, Select, SelectOption } from '@dm3/ui';
...
export function DepartmentModal({ isOpen, onClose, onSubmit, department, title }: DepartmentModalProps) {
    ...
    const loadManagers = async () => {
        try {
            const response = await apiFetch<{ managers: DepartmentManager[] }>('/api/v1/identity/departments/managers');
    ...
    const loadParentDepartments = async () => {
        try {
            ...
            const response = await apiFetch<{ departments: Department[] }>(`/api/v1/identity/departments?${params}`);
