...
export function UserModal({ isOpen, onClose, onSave, user }: UserModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const savedId = typeof saved === 'object' && saved !== null && 'id' in saved ? (saved as { id: string | number }).id : user?.id;
    ...
    const initials = formData.first_name && formData.last_name ? `${formData.first_name[0]}${formData.last_name[0]}`.toUpperCase() : null;
    ...
    return (
        <AppModal
            ...
            primaryAction={{
                label: loading ? t('modal.saving') : t('modal.save'),
                onClick: handleSave,
                disabled: loading,
                loading,
            }}
        >
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <Label htmlFor="first_name">
                            {t('modal.firstName')} <span className="text-destructive">*</span>
                        </Label>
                        <Input
                            id="first_name"
                    ...
                    <div className="space-y-1">
                        <Label htmlFor="last_name">
                            {t('modal.lastName')} <span className="text-destructive">*</span>
                        </Label>
                        <Input id="last_name" value={formData.last_name} onChange={set('last_name')} placeholder="Doe" required disabled={loading} />
                    </div>
                </div>
                ...
                <div className="space-y-1">
                    <Label htmlFor="email">
                        {t('modal.email')} <span className="text-destructive">*</span>
                    </Label>
                    <Input
                        id="email"
                        type="email"
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <Input id="position" value={formData.position} onChange={set('position')} placeholder="Developer" disabled={loading} />
                    </div>
                    <div className="space-y-1">
                        <Label htmlFor="phone">{t('modal.phone')}</Label>
                        <Input id="phone" value={formData.phone} onChange={set('phone')} placeholder="+84..." disabled={loading} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <Input id="birth_day" type="date" value={formData.birth_day} onChange={set('birth_day')} disabled={loading} />
                    </div>
                    <div className="space-y-1">
                        <Label>{t('modal.gender')}</Label>
                        <Select value={formData.sex} onValueChange={(v) => setFormData((prev) => ({ ...prev, sex: v as '' | 'true' | 'false' }))}>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <Input id="emp_number" value={formData.emp_number} onChange={set('emp_number')} placeholder="EMP-001" disabled={loading} />
                    </div>
                    <div className="space-y-1">
                ...
                <div className="space-y-1">
                    ...
                    <Input id="address" value={formData.address} onChange={set('address')} placeholder="123 Main St..." disabled={loading} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <Input id="effective_date" type="date" value={formData.effective_date} onChange={set('effective_date')} disabled={loading} />
                    </div>
                    <div className="space-y-1">
                        <Label htmlFor="expired_date">{t('modal.expiredDate')}</Label>
                        <Input id="expired_date" type="date" value={formData.expired_date} onChange={set('expired_date')} disabled={loading} />
                    </div>
                </div>
