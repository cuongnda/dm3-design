...
function CreateCompanyPageContent() {
  ...
  const isFormValid = form.name.trim() && form.code.trim() && form.email.trim();
      ...
      <div className="p-6 max-w-lg mx-auto mt-12">
        <div className="border border-border rounded-lg p-6 bg-card">
          ...
          <p className="text-[13px] text-muted-foreground text-center mb-6">
            <span className="text-foreground font-medium">{result.company?.name || 'Company'}</span> has been set up successfully.
          </p>

          {result.credentials ? (
            <div className="bg-background border border-operate/30 rounded-md p-4 mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] uppercase tracking-wider text-operate font-medium">{safeT('createCompany.credentials', 'Credentials')}</span>
            ...
            <div className="bg-background border border-operate/30 rounded-md p-4 mb-4">
              <p className="text-[13px] text-muted-foreground">Company created successfully. Admin credentials will be provided separately.</p>
            </div>
          )}
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <h1 className="text-[20px] font-semibold text-foreground mb-6">{safeT('', '')}</h1>
  ...
  return (
    <ErrorBoundary
      onError={(error, errorInfo) => {
        console.error('CreateCompanyPage Error:', error, errorInfo);
      }}
    >
