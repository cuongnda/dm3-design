...
function RowDetail({ entry, t }: RowDetailProps) {
  return (
    <tr>
      <td colSpan={7} className="px-4 py-3 bg-card/30">
      ...
      <div className="bg-card border border-border rounded-lg p-3">
        <div className="flex flex-wrap gap-2 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-muted-foreground">{t('audit.filters.service')}</label>
            <select
              value={draftFilters.service ?? ''}
              onChange={(e) => setDraftFilters((prev) => ({ ...prev, service: e.target.value || undefined }))}
          ...
          {/* Entity Type */}
          <div className="flex flex-col gap-1">
            <label className="text-[11px] text-muted-foreground">{t('audit.filters.entityType')}</label>
            <select
              value={draftFilters.entity_type ?? ''}
              onChange={(e) => setDraftFilters((prev) => ({ ...prev, entity_type: e.target.value || undefined }))}
              className="bg-card border border-border rounded-md px-3 py-1.5 text-[13px] text-foreground"
            >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-4 py-2.5 text-left text-muted-foreground text-[11px] uppercase tracking-wider font-medium">{t('audit.table.entity')}</th>
                  ...
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-[13px] text-foreground">
                        <div>{entry.actor_email ?? entry.actor_id ?? '—'}</div>
                      </td>
