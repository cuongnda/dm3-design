...
export default defineConfig({
    ...
    server: {
        ...
        proxy: {
            ...
            '/cctv/whep/': { target: 'http://localhost:8889', changeOrigin: true, rewrite: (path) => path.replace(/^\/cctv\/whep/, '') },
            '/cctv/hls/': { target: 'http://localhost:8888', changeOrigin: true, rewrite: (path) => path.replace(/^\/cctv\/hls/, '') },
