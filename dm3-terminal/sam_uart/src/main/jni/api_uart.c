...
void ApiUartClose(int uartCh)
{
	...
	if(devUART[uartCh] < 0) {
		DbgPrintf("Uart[%d] is already closed%d!!\n",uartCh,devUART[uartCh]);
...
int ApiUartInitBuffer(int uartCh)
{
	...
	if(result < 0) {
		DbgPrintf("Uart Init Buffer FAILURE [%d]\n", result);
		return result;
	}
	else {
		return result;
...
int ApiUartCheckRxCnt(int uartCh)
{
	...
	if(result < 0) {
		printf("ApiUartCheckRxCnt FAILURE [%d]\n", result);
		return result;
	}
...
#if 0
int qrApiUartCheckRxCnt(int uartCh)
{
	int result;
	int inbyte;
...
int ApiUartConfigXDisableRx(int uartCh)
{
	...
	rv = tcflow(devUART[uartCh], TCIOFF);
	if(rv<0)
	{
		printf("ApiUartConfigXDisableRx Error\n",uartCh);
		close(devUART[uartCh]);
		devUART[uartCh] = -1;
		return -1;
	}	
...
int ApiUartConfigXEnableRx(int uartCh)
{
	...
	rv = tcflow(devUART[uartCh], TCION);
	if(rv<0)
	{
		printf("ApiUartConfigXEnableRx Error\n",uartCh);
		close(devUART[uartCh]);
		devUART[uartCh] = -1;
		return -1;
	}	
