...
def _get_module_web_titles(result: dict) -> list:
    """Get WebTestExecutor test case titles for a web test module.
    
    Reads the pytest JSON report to find test function names,
    then reads the data JSON file to map function names → titles.
    """
    titles = []
    ...
    try:
        rj = json.loads(Path(report_json).read_text())
    except Exception:
        return titles
    
    # Find the test file and data file
    test_file = None
    for t in rj.get("tests", []):
        nodeid = t.get("nodeid", "")
        if "/web/" in nodeid:
            # e.g., tests/web/system-admin/test_company_management.py::test_xxx
            parts = nodeid.split("::")
            test_file = parts[0]
            break
    ...
    # tests/web/system-admin/test_company_management.py 
    # → data/web/system-admin/test_company_management.data.json
    data_path = test_file.replace("tests/", "data/").replace(".py", ".data.json")
    data_full = PROJECT_DIR / data_path
    ...
    # Also scan execution_reports for title-named files created during this test
    # These were generated by WebTestExecutor
    exec_dir = EXPORT_DIR / "execution_reports"
    if exec_dir.exists():
        # Get test function names from pytest report
        func_names = set()
        for t in rj.get("tests", []):
            parts = t.get("nodeid", "").split("::")
            if len(parts) >= 2:
        ...
        # Match execution report files by title similarity
        for f in exec_dir.iterdir():
            if f.is_file() and f.suffix == '.json' and not f.name.startswith('execution_steps_TC_'):
                # Already have this title from data file
                name = f.stem
                if name.startswith('execution_steps_'):
                    import re as _re3
                    rest = _re3.sub(r'_\d{8}_\d{6}$', '', name[len('execution_steps_'):])
                    if rest and rest not in [t.replace(" ", "_") for t in titles]:
                        titles.append(rest.replace("_", " "))
    
    return titles


def _generate_module_artifacts(result: dict):
    ...
    # Collect all JSON files for this module
    module_json_files = []
    
    if exec_dir.exists():
        # 1) TC_ prefixed files (from _generate_execution_reports)
        for f in exec_dir.iterdir():
            if f.is_file() and f.suffix == '.json' and tc_prefix in f.name:
                module_json_files.append(str(f))
        
        # 2) For web modules: also find WebTestExecutor title-named files
        if is_web:
            web_titles = _get_module_web_titles(result)
            for f in exec_dir.iterdir():
                if f.is_file() and f.suffix == '.json' and not f.name.startswith('execution_steps_TC_'):
                    fname = f.stem
                    if fname.startswith('execution_steps_'):
                        import re as _re4
                        rest = _re4.sub(r'_\d{8}_\d{6}$', '', fname[len('execution_steps_'):])
                        # Match by title
                        rest_readable = rest.replace("_", " ")
                        for title in web_titles:
                            if title.replace(" ", "_") == rest or rest_readable == title:
                                module_json_files.append(str(f))
                                break
    ...
    # WebTestExecutor JSONs have real screenshot paths → video works
    # TC_ JSONs from API tests have empty paths → naturally skipped
    video_count = 0
    if os.environ.get("GENERATE_VIDEO", "true").lower() == "true":
        try:
            ...
            for jf in module_json_files:
                try:
                    vid = generate_video_for_report(jf)
                    if vid:
                        video_count += 1
                except Exception:
                    pass
...
def run_all_tests(test_type: str = "all", upload: bool = True) -> dict:
    ...
    for test_file in test_files:
        ...
        _generate_module_artifacts(result)
...
def run_module(module_name: str, upload: bool = True) -> dict:
    ...
    for test_file in sorted(set(test_files)):
        ...
        # Generate step reports + videos
        _generate_module_artifacts(result)

        # Inject links into pytest HTML report
        report_html = EXPORT_DIR / f"report_{result['module']}.html"
        _inject_report_links(report_html, result["module"])
