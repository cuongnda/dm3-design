...
Adapted from dmpw_automation/generate_step_report.py
"""
import json
import os
import glob
...
def generate_html_report(json_file_path: str) -> str:
    ...
    for step in report['steps']:
        ...
        params_json = json.dumps(step.get('params', {}), indent=2)
        data_json = json.dumps(step.get('data_captured', {}), indent=2)

        # Screenshots
        shots = ""
        for key, label in [('screenshot_before', 'Before'), ('screenshot_after', 'After')]:
            if step.get(key) and os.path.exists(step[key]):
                rel = os.path.relpath(step[key], os.path.dirname(json_file_path))
        ...
        video_btn = ""
        if video_path:
            video_btn = f'<button class="vid-btn" onclick="jumpToStep({step_time_offset})">Video {step_time_offset}s</button>'
        step_time_offset += 4

        steps_html += f"""
        <div class="step">
          <div class="step-header {status_class}">
            <span class="phase-badge phase-{step['phase']}">{step['phase'].upper()}</span>
            <strong>Step {step['counter']}: {step['action']}</strong>
          ...
          <div class="step-content">
            <details><summary>Parameters</summary><pre class="code">{params_json}</pre></details>
            <details><summary>Data Captured</summary><pre class="code">{data_json}</pre></details>
            {error_html}
            {shots}
          </div>
        </div>"""
...
.step {{ margin-bottom: 16px; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; }}
...
.step-meta {{ margin-left: auto; display: flex; align-items: center; gap: 10px; }}
...
.error-box pre {{ margin: 4px 0 0; font-size: 12px; white-space: pre-wrap; }}
.phase-badge {{ padding: 3px 8px; border-radius: 4px; font-size: 11px; color: #fff; }}
...
.vid-btn {{ font-size: 11px; padding: 2px 8px; background: #2196F3; color: #fff; border: none; border-radius: 4px; cursor: pointer; }}
.video-section {{ text-align: center; margin-bottom: 24px; }}
.video-open-btn {{ padding: 10px 24px; background: #2196F3; color: #fff; border: none; border-radius: 6px; font-size: 15px; font-weight: 600; cursor: pointer; }}
...
.video-controls {{ padding: 10px 20px; background: #222; text-align: center; }}
...
<div class="container">
  <div class="header">
    <h1>Step-by-Step Execution Report</h1>
    <p><strong>Test Case:</strong> {tc_id} {('- ' + title) if title else ''}</p>
    <p><strong>Time:</strong> {report['execution_timestamp']}</p>
  </div>
  <div class="summary">
    <div class="summary-card"><h3>Result</h3><div class="val" style="color:{result_color}">{result_text}</div></div>
    <div class="summary-card"><h3>Steps</h3><div class="val">{report['total_steps']}</div></div>
    <div class="summary-card"><h3>Completed</h3><div class="val" style="color:#28a745">{report['steps_completed']}</div></div>
    <div class="summary-card"><h3>Failed</h3><div class="val" style="color:#dc3545">{report['steps_failed']}</div></div>
    <div class="summary-card"><h3>Duration</h3><div class="val">{report['total_duration']:.1f}s</div></div>
  </div>
  {video_section}
